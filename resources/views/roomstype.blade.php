...
<body class="bg-gray-200">
    ...
    Bienvenue à notre hôtel ! Nous sommes ravis de vous accueillir dans notre établissement <br>où vous découvrirez un monde de confort et d'élégance. Explorez nos chambres exceptionnelles conçues pour offrir une expérience de séjour inoubliable. </p>
            @foreach($roomstype as $key => $room)
                <div class="room{{ $key + 1 }} max-w-sm rounded overflow-hidden shadow-lg"id="card{{ $key + 1 }}">
                    <img class="images w-full" src="{{ $room->image }}" alt="{{ $room->type }}" id="r{{ $key + 1 }}">
                    <div class="px-6 py-4">
                        <div class="titre{{ $key + 1 }}">       
                           <h3 id="titre{{ $key + 1 }}">{{ $room->type }}</h3></div>
                        ...
                        <div class="btn">
                            <a href="roomdetail{{ $room->id }}" class="m11 lien1" id="see{{ $key + 1 }}">See more</a>
                        </div>
                    </div>
                </div>
            @endforeach
