...
<body>
    <div class="room-detail w-96 md:w-5/6 h-96 md:h-auto p-8 md:p-0" id="room">
        <div class="md:mr-8">
            <img src="{{ asset($room1->image) }}" alt="image1" id="img1" class="mb-4 md:mb-0 md:mr-4 w-72 h-72">
        </div>
        <div class="md:flex-grow" id="info">
            <div class="room1">
                <h3 class="text-xl font-bold mb-2 font-semibold " id="name">Room name</h3>
                <h3 class="text-1xl font-bold mb-2 text-600 font-serif" id="name1">{{ $room1->type }}</h3>
            </div>
            <div class="room11">
                <h3 class="text-xl font-bold mb-2" id="price1">Price</h3>
                <h3 class="text-1xl font-bold mb-2 text-blue-600 font-serif" id="price2">{{ $room1->prix }}/day</h3>
            </div>
            <hr class="trait">
            <div class="service1">
            ...
            <div class="btn">
                    <form action="{{route('verifydate')}}" method="GET">
                        ...
                        @if((Session::has('erreur')) && Session::get('erreur') === "la chambre n est pas disponible")
                        <div class="warning p-4 mb-4 text-sm text-yellow-800 rounded-lg bg-yellow-50 dark:bg-gray-800 dark:text-yellow-300" role="alert">
                           <p><span class="font-medium">Warning alert!</span>cette chambre n'est pas disponible</p>
                            <script>
                    ...
                    @if((Session::has('erreur')) && Session::get('erreur') !== "la chambre n est pas disponible")
                        <div class="success p-4 mb-4 text-sm text-green-800 rounded-lg bg-green-50 dark:bg-gray-800 dark:text-green-400" role="alert">
                            <span class="font-medium">Success alert!</span> cette Chambre est  disponible.
                        </div>
                    @endif
                </div>
            </div>
